...
给定几个王国的坐标位置（通过指定王国的房屋位置和发电厂的位置）和导弹打击位置，编写一个程序，确定所有王国在导弹火力轰炸之后没有电力的总面积。
在这个问题的简单世界里，王国并不重叠。此外，每个王国周围的墙壁被认为是零厚度的。王国周围的墙是最小周长边，它完全包围了包括王国的所有房屋和发电站；王国的区域是由最小周长边围起来的区域。
每个王国只有一个发电站。
王国之间可能存在空白的空间。
