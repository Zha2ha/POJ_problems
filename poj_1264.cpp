...
当战争的冲动变得太强烈时，王国人民经常向其他王国发射导弹。每一枚飞毛腿导弹（动物学倾向的全球性攻击者）落入一个王国的城
墙内，摧毁了这个王国的发电厂（没有生命损失）。
给定几个王国的坐标位置（通过指定王国的房屋位置和发电厂的位置）和导弹着陆，您将编写一个程序，确定所有王国在交换导弹火力
之后没有电力的总面积。
在这个问题的简单世界里，王国并不重叠。此外，每个王国周围的墙壁被认为是零厚度的。王国周围的墙是最小周边墙，它完全包围了
包括王国的所有房屋和发电站；王国的区域是由最小周边薄壁围起来的区域。
每个王国恰好有一个发电站。
王国之间可能存在空白的空间。
...
missile attacks. 
输入是王国规格的序列，然后是导弹着陆位置的序列。
一个王国是由一行上的数字N（3<=N<=100）指定的，它表示这个王国的站点数量。下一条线包含电站的x和y坐标，接下来的N-1行
的x,y坐标表示该电站供电的住宅位置。N为-1表示不再有王国。至少有一个王国。
在最后一个王国的说明之后，将是一个或多个导弹攻击的坐标，指示导弹着陆的位置。每个导弹位置占一行。您将处理导弹攻击，
直到您到达文件的结尾。
使用500公里乘500公里网格上的坐标以公里为单位指定位置。所有坐标都是0到500之间的整数。坐标被指定为一行上一对由空格分
隔的整数。输入文件将由多达20个王国组成，以及任何数量的导弹攻击。
...
places.
输出由单个数字组成，表示所有王国在所有导弹攻击后以后没有电力的总面积。数字应该输出为（或更正为）两位小数的格式。
